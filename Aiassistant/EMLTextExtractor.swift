...
enum EMLTextExtractor {
    ...
    private static func decodedPartBody(_ body: String, headers: String, contentType: String) -> String {
        ...
        if transfer == "base64",
           let data = Data(base64Encoded: trimmed.replacingOccurrences(of: "\\s", with: "", options: .regularExpression),
    ...
    private static func decodeMIMEWords(_ value: String) -> String {
        ...
        for match in matches {
            ...
            if let decoded {
                output.replaceSubrange(whole, with: decoded)
            }
        }
