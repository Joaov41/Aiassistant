...
class AppDelegate: NSObject, NSApplicationDelegate { // No NSWindowDelegate needed here
    ...
    private var tapTimer: Timer? // Timer to handle timeouts

    // Event monitors for keyboard events. Global handles other apps; local handles Aiassistant while it is frontmost.
    private var globalEventMonitor: Any?
    private var localEventMonitor: Any?
    private var gemmaServerTask: Task<Void, Never>?
    private var startMLXServersMenuItem: NSMenuItem?
    private var stopMLXServersMenuItem: NSMenuItem?
    private var isStartingMLXServers = false
    ...
    private func setupMenuBar() {
        ...
        let startServersItem = NSMenuItem(title: "Start MLX Servers", action: #selector(startMLXServersFromMenu), keyEquivalent: "")
        startServersItem.target = self
        menu.addItem(startServersItem)
        startMLXServersMenuItem = startServersItem
        ...
        stopServersItem.isEnabled = false
    ...
    func showQuickActionsPopup() {
        ...
        let window = QuickActionsWindow(appState: appState, inlineReplacementMode: true)
    ...
    @objc private func showSettings() {
        ...
        if let existingWindow = self.settingsWindow {
             print("Settings window already exists. Activating.")
    ...
    @objc private func startMLXServersFromMenu() {
        guard !isStartingMLXServers else { return }
        isStartingMLXServers = true
        startMLXServersMenuItem?.isEnabled = false
        startMLXServersMenuItem?.title = "Starting MLX Servers..."

        gemmaServerTask = Task { [weak self, appState] in
            do {
                try await appState.coreAIGemmaProvider.startServerIfNeeded()
                await MainActor.run {
                    self?.isStartingMLXServers = false
                    self?.gemmaServerTask = nil
                    self?.startMLXServersMenuItem?.title = "Start MLX Servers"
                ...
                await MainActor.run {
                    ...
                    self.gemmaServerTask = nil
                    self.startMLXServersMenuItem?.title = "Start MLX Servers"
                    self.startMLXServersMenuItem?.isEnabled = true

                    let alert = NSAlert()
                    alert.messageText = "Could Not Start MLX Servers"
    ...
    @objc private func stopMLXServersFromMenu() {
        ...
        Task { [weak self, appState] in
            ...
            await MainActor.run {
                ...
                self?.stopMLXServersMenuItem?.isEnabled = false
                self?.startMLXServersMenuItem?.isEnabled = true
            }
        }
    }
    ...
    private func setupShiftTapMonitor() {
        localEventMonitor = NSEvent.addLocalMonitorForEvents(matching: .flagsChanged) { [weak self] event in
            self?.handleShiftEvent(event)
            return event
        }

         // Ensure AccessibilityHelper is defined and working for events in other apps
         guard AccessibilityHelper.checkAccessibilityPermissions() else {
             print("Accessibility permissions not granted. Global shift tap monitor not set up.")
    ...
    private func handleShiftEvent(_ event: NSEvent) {
        let shiftPressed = event.modifierFlags.contains(.shift)
        let isLeftShiftEvent = event.keyCode == 56 // Left Shift physical keycode

        if isLeftShiftEvent && shiftPressed {
            handleShiftPress()
        }
    }
    ...
    private func handleShiftPress() {
        ...
        case .idle:
            currentTapState = .firstPress(time: now)
        ...
        case .firstPress(let t1):
            if now.timeIntervalSince(t1) < tapInterval {
                currentTapState = .secondPress(time: now)
                ...
                currentTapState = .firstPress(time: now) // Reset
        ...
        case .secondPress(let t2):
            if now.timeIntervalSince(t2) < tapInterval {
